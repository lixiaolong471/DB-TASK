...
public class QueryThreadPoolExecutor {
	...
	public static ExecutorService newFixedThreadPoolExecutor() {
		pool = new ThreadPoolExecutor(Context.getGetPool()+Context.getSavePool(), Context.getGetPool()+Context.getSavePool(), 30, TimeUnit.MINUTES,
				new ArrayBlockingQueue<Runnable>(Context.getGetPool()+Context.getSavePool()),
	...
	private static class CustomThreadFactory implements ThreadFactory {
		...
		public Thread newThread(Runnable r) {
			...
			String threadName = QueryThreadPoolExecutor.class.getSimpleName() + count.addAndGet(1);
